...
export default function CameraUpload({ appliances, constraints, onResult }) {
  ...
  const processFile = (file) => {
    ...
    // Simulate API Call - Replace this with your actual prediction logic later!
    setTimeout(() => {
      // Mock result to trigger the UI update
      const mockResult = {
        detected_food: "Avocado Toast",
        confidence: 0.96,
        food_info: {
            calories: 320,
            protein: 12,
    ...
    <div 
      className={`relative border-2 border-dashed rounded-3xl p-12 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[350px]
        ${isDragging 
          ...
          : 'border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-[#0B0F19]' // Matches the dark background from Figma
        }
      `}
    ...
    >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center animate-fade-in pointer-events-none">
          {/* Green Camera Icon Box from Figma */}
          ...
          {/* Choose File Button from Figma */}
          <button 
            onClick={() => fileInputRef.current.click()}
            className="pointer-events-auto bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-900 dark:text-white px-6 py-2.5 rounded-xl font-medium transition-colors flex items-center gap-2 border border-transparent dark:border-gray-700"
          >
